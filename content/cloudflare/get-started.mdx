...
### Add an `open-next.config.ts` file
...
```json filename="package.json" {4-7}
{
  "scripts": {
    "build": "next build",
...
- The `build` script must invoke the Next.js build command, it will be invoked by [`opennextjs-cloudflare build{:sh}`](/cloudflare/cli#build-command).
- `npm run preview{:sh}`: Builds your app and serves it locally, allowing you to quickly preview your app running locally in the Workers runtime, via a single command.
- `npm run deploy{:sh}`: Builds your app, and then immediately deploys it to Cloudflare.
- `npm run upload{:sh}`: Builds your app, and then uploads a new [version](https://developers.cloudflare.com/workers/configuration/versions-and-deployments/#versions) of it to Cloudflare.
- `npm run cf-typegen{:sh}`: Generates a `cloudflare-env.d.ts` file at the root of your project containing [the types for the `env`](https://developers.cloudflare.com/workers/wrangler/commands/#types).
...
### Remove any `export const runtime = "edge";{:js}` if present

Before deploying your app, remove the `export const runtime = "edge";{:js}` line from any of your source files.
...
### Remove `@cloudflare/next-on-pages` (if necessary)
...
In step 3, we also added the `npm run preview{:sh}`, which allows you to quickly preview your app running locally in the Workers runtime,
