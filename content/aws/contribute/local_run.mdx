...
When making some changes to OpenNext, it can be a bit cumbersome to need to deploy every time you want to test changes.

If your change is not dependent on the wrapper or the converter, then you can create a custom `open-next.config.ts` file (you can use another name so that it doesn't conflict with your existing `open-next.config.ts`).
